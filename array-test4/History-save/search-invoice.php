...
if($role == 'Admin') {
    ...
    $sql = "SELECT * FROM client_invoice WHERE compName LIKE '%$searchTerm%' ORDER BY id DESC";
} else {
    // If the user is not an admin, only search their own records
    $sql = "SELECT * FROM client_invoice WHERE compName LIKE '%$searchTerm%' AND username='$username' ORDER BY id DESC";
}
$result = $conn->query($sql);
